        ...
        {voteItems?.map((item, idx) => (
          ...
          >
            <div className="flex items-center justify-between mb-2 gap-2">
              ...
              {item.importance && (
                <span className="shrink-0 px-2 py-0.5 bg-red-400 text-white text-[10px] rounded-full animate-pulse">
                  중요
                </span>
              )}
            </div>
            ...
            <a 
              ...
              target="_blank" 
              rel="noreferrer"
              className={`block w-full py-2 text-center text-sm text-white rounded-lg transition-all mt-auto ${
                item.type === 'hype' 
                  ? 'bg-red-500 hover:bg-red-600' 
                  : 'bg-gradient-to-r from-purple-400 to-pink-400 hover:from-purple-500 hover:to-pink-500'
              }`}
            >
              {item.type === 'hype' ? 'Hype 하기' : '투표 하기'}
            </a>
          </div>
        ))}
      </div>
